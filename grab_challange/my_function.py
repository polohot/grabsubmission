...
def make_label(df_in,df_map):
    ...
    df_in: input of unlabelled dataframe
    df_map: mapper dataframe
    """
    ls_label=[]
    for i in range(0,len(df_in)):
        i_cat = df_in['cat'][i]
...
def transfer_learner(source,learn,tfms,sz,bs,cycle,lr=1e-3):
    ...
    source: input of image source
    learn: input trained or loaded model
    tfms: image augmentation transform function
    sz: image size to train with
    bs: batch size, reduce if out of memory
    cycle: cycles to train the model
    lr: learning rate
    """
...
def calc_accuracy(learn,df_in):
    ...
    learn: trained or loaded model
    df_in: dataframe to predict
    """
    ls_prd=[]
    ls_real=[]
    ...
    for i in range(0,len(df_in)):
        ...
        if (i+1)%10 == 0:
            ...
            progress(i,len(df_in),'current accuracy '+str(round(correct,4)))            
    score = pd.DataFrame({'prd':ls_prd,'act':ls_real})
    correct = np.where(score['prd'] == score['act'],1,0).sum() / len(score)  
